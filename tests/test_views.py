...
def create_method_instance(url, page_name, template, model, creation_attrs):
    def method(self):
        ...
        self.assertTemplateUsed(response, template)
        self.assertEqual(
            self.client.get(created_reversed_url).status_code,
            status.HTTP_200_OK)

    return method


instance_pages = (('/category/',
                   'category',
...
methods_with_auth = {
    f'test_{page[1]}': create_method_with_auth(
        *page, login=True) for page in pages}
TestWithAuth = type('TestWithAuth', (TestCase,), methods_with_auth)

casual_methods = {
    f'test_with_auth_{page[1]}': create_method_with_auth(
        *page, login=True) for page in casual_pages}
casual_methods.update({f'test_no_auth_{page[1]}': create_method_with_auth(
    *page, login=False) for page in casual_pages})
TestCasualPage = type('TestCasualPages', (TestCase,), casual_methods)

methods_no_auth = {
    f'test_{url}': create_method_no_auth(url) for url,
    _,
    _ in pages}
TestNoAuth = type('TestNoAuth', (TestCase,), methods_no_auth)

methods_intance = {
    f'test_{page[1]}': create_method_instance(
        *page) for page in instance_pages}
